...
# First replace Cloudflare block
cf_pattern = r'<div className="glass-card">\s*<h3[^>]*>.*?Cloudflare Tunnel Settings.*?</form>\s*</div>'
content = re.sub(cf_pattern, '', content, flags=re.DOTALL)
...
# Insert SMTP card at the very end before the closing tags
content = content.replace("                    </div>\n    );\n};", smtp_card + "\n                    </div>\n    );\n};")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print('Done applying regex and injecting SMTP block')
